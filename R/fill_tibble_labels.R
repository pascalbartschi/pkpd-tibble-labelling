...
fill_tibble_labels <- function(tbble, mode = "remote") {
  ...
  if (mode == "remote"){
    labels <- read.csv("https://raw.githubusercontent.com/pascalbartschi/pkpd-tibble-labelling/main/output/all_variables2labels.csv") %>%  pull(2, 1) # converts df to names list
  } else {
